...
function runpf(mpc, opt::Dict{String})
    ...
    # options
    qlim::Bool = opt["PF"]["ENFORCE_Q_LIMS"];         ## enforce Q limits on gens?
    dc::Bool = opt["PF"]["DC"];  ## use DC formulation?  

    # Step 2.2: Convert the data into the required format
    baseMVA::Float64 = mpc["baseMVA"];
    bus::Matrix{Float64} =  mpc["bus"];
    gen ::Matrix{Float64}= mpc["gen"];
    branch::Matrix{Float64} = mpc["branch"];
    ...
    if(dc==1)
        ...
        while (repeat>0)
            ...
            Sbus_gpu=Vm_gpu->makeSbus_gpu(baseMVA, bus, gen, Vm_gpu);
            ## run the power flow is NR is selected
            if alg == "NR"
                V, success, iterations = newtonpf(Ybus, Sbus_gpu, V0, ref, pv, pq, opt["PF"]["PF_TOL"], opt["PF"]["PF_MAX_IT"], opt["PF"]["NR_ALG"]);
                its += iterations;
            end
            ## update data matrices with solution
